...
# Buscar y ejecutar launcher.exe en las carpetas especificadas
$launcherPaths = @(
    "$env:USERPROFILE\Documents\launcher.exe",
    "$env:USERPROFILE\Documents\launcher.exe",
    "$env:USERPROFILE\OneDrive\Documentos\launcher.exe",
    "$env:USERPROFILE\OneDrive\Documents\launcher.exe"
)
...
# Eliminar launcher.exe al cerrar el script
if ($launcherFound) {
    foreach ($path in $launcherPaths) {
        if (Test-Path -Path $path) {
